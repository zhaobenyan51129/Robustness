...
size:图片大小
'''
def generate_input(path,C,P,SF,D,frameRate,size,stimulus_name):
      n = 1 # 给图片编号的序号
      C = C.astype('float32')
      P = P.astype('float32')
...
def Get_fStimulus_Name(path,stimulus_name):
      file_names = os.listdir(path)
      fStimulus = []
      for file_name in file_names:
            if file_name.startswith(f"{stimulus_name}") and file_name.endswith(".bin") and not file_name.endswith("_cfg.bin"):
